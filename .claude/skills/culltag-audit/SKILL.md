...
sneaking, or that T is on a team with `nametagVisibility=NEVER`. That lie is state living in
two places at once, on the server and inside V's client, and every shipped bug has been the
two disagreeing.
...
- Fixed after 1.1.1: an override was set on a pair and then the pair went out of
  ...
  off. They stayed crouched and nameless at range.
...
- **One record of what the client was told, and it lives on the connection.** The hidden sets
  on `NametagController` *are* the state; nothing mirrors them in a map keyed by UUID.
...
| Config field never read outside `CullTagConfig` | This is the static shadow of the `crouch_hides_nametag` bug: fully wired into the config machinery, connected to nothing. |
...
believes because CullTag told it to. Adding a third is fine. What is not fine is adding one
...
- **Hidden entity IDs** need no restore packet when the player leaves, because the client is
  about to remove that entity anyway. They do need clearing on respawn, because a respawned
  player keeps their connection and becomes a **new entity with a new ID**; the old one would
  otherwise sit in every viewer's set for the rest of the session.
- **Crouch-hidden names** always need the explicit removal packet, because scoreboard team
  membership is keyed by **name** and a client keeps its team roster across a player leaving.
  Skip it and that player comes back already hidden.
...
- The cases that matter are the ones where the override has to come *off*, and none of them is
  visible from one client. Two players, one behind a wall: walk out past `max_distance` and
  confirm the nametag comes back, rather than the player staying hunched over. Log the hidden
  player out and back in. Kill them and let them respawn. Toggle `/culltag disable` and
  `/culltag enable` while a pair is blocked, and confirm the pair is hidden again on the next
  sweep rather than only after somebody moves.
- Glass is a `COLLIDER`, so the raycast treats it as a wall. That is a design question, not a
  bug the audit can see, but it is the thing most likely to be reported as one.
- `/culltag stats` is the cheap health check. `raycasts` climbing while `LOS-hidden` stays at
